...
- **Resi tidak bisa diterbitkan dari sini, hanya dilacak.** RajaOngkir tidak punya endpoint pembuatan pengiriman — `/order`, `/shipment`, `/booking`, dan `/pickup` semuanya 404. Resi lahir saat kurir menerima fisik paketnya. Yang tersedia adalah `POST track/waybill`: diberi resi yang sudah ada, ia mengembalikan posisi paket. Dipakai tombol "Cek posisi paket" di menu Pengiriman, dan order ditandai Selesai lewat `MarkDelivered` yang sudah ada — bukan jalur tersendiri — supaya aturan transisi dan catatan auditnya sama dengan penandaan manual. **Status "terkirim" hanya diakui kalau kurir menyatakannya sendiri**; menebaknya dari riwayat perjalanan berarti order bisa jadi Selesai padahal paketnya masih di jalan. Resi yang baru diserahkan sering belum dikenali sistem kurir, jadi jawaban "Invalid Awb" bukan bukti salah ketik dan tidak boleh membatalkan apa pun.
