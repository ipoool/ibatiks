 ...
 *
 * Tab ini menjawab pertanyaan "hari ini saya harus mengemas apa saja", yang
 * tidak enak dicari lewat daftar order umum. Duduk sebaris dengan daftar paket
 * karena keduanya satu pekerjaan: mengemas lalu menyerahkannya ke kurir.
 */
...
type AntreanStatus = (typeof QUEUE_OPTIONS)[number]["value"];

/*
 * Keterangan tiap antrean. Tipenya diturunkan dari QUEUE_OPTIONS supaya
 * keduanya tidak bisa berjalan sendiri-sendiri: sebelumnya di sini ada kunci
 * "arrived" yang tidak pernah muncul di pilihan, sementara "dp_paid" — antrean
 * yang terbuka pertama kali — justru tidak punya keterangan sama sekali dan
...
export function AntreanKemas() {
  const [page, setPage] = useState(1);
  const [status, setStatus] = useState<AntreanStatus>("dp_paid");
