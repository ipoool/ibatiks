...
type receiveItemRequest struct {
	...
	QtyReceived int       `json:"qty_received" validate:"gte=0"`
	Status      string    `json:"status"       validate:"omitempty,oneof=pending purchased partial unavailable refunded"`
}

type receiveRequest struct {
	Items []receiveItemRequest `json:"items" validate:"required,min=1,dive"`
}

// Receive mencatat pencocokan barang yang datang dengan isi pesanan.
func (h *OrderHandler) Receive(w http.ResponseWriter, r *http.Request) {
	...
	receipts := make([]service.ItemReceipt, 0, len(req.Items))
	for _, item := range req.Items {
		receipts = append(receipts, service.ItemReceipt{
			ItemID:      item.ItemID,
	...
	order, err := h.orders.ReceiveItems(r.Context(), orderID, receipts, middleware.UserIDFrom(r.Context()))
	if err != nil {
		response.Error(w, r, err)
		return
	}
